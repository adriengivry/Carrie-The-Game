...
{
	EventInfo() { m_code = 0; }
	explicit EventInfo(const int l_event) { m_code = l_event; }
	union
	{
		int m_code;
	};
...
struct Binding
{
	explicit Binding(const std::string& l_name) : m_name(l_name), c(0), m_details(l_name) {}
	~Binding() {}
...
public:
	...
	bool AddCallback(StateType l_state, const std::string& l_name,
		void(T::*l_func)(EventDetails*) const, T* l_instance)
	{
	...
	bool RemoveCallback(const StateType l_state, const std::string& l_name)
	{
		...
		const auto itr2 = itr->second.find(l_name);
