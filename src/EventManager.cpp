...
void EventManager::HandleEvent(sf::Event& l_event)
{
	...
	for (auto &b_itr : m_bindings)
	{
		...
		{
			const EventType sfmlEvent = static_cast<EventType>(l_event.type);
...
void EventManager::Update()
{
	...
	for (auto &b_itr : m_bindings)
	{
		...
		if (bind->m_events.size() == bind->c)
		{
			...
			{
				const auto callItr = stateCallbacks->second.find(bind->m_name);
			...
			{
				const auto callItr = otherCallbacks->second.find(bind->m_name);
...
void EventManager::LoadBindings()
{
	...
	bindings.open(Utils::loadConfig("keys.cfg"));
	if (!bindings.is_open())
	{
	...
	while (std::getline(bindings, line))
	{
		...
		while (!keystream.eof())
		{
			...
			const int code = stoi(keyval.substr(end + delimiter.length(),
